...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--summary-json", type=Path, required=False, default=None)
...
def validate_ids(input_rows: List[Dict[str, Any]], output_rows: List[Dict[str, Any]]) -> None:
    ...
    if set(in_ids) != set(out_ids):
        missing = sorted(set(in_ids) - set(out_ids))
        extra = sorted(set(out_ids) - set(in_ids))
        raise SystemExit(f"ID mismatch. missing={missing[:5]} extra={extra[:5]}")


def validate_embeddings(output_rows: List[Dict[str, Any]]) -> int:
    if not output_rows:
        raise SystemExit("No output rows found")
    first = output_rows[0].get("embedding")
...
def validate_generation(output_rows: List[Dict[str, Any]]) -> None:
    if not output_rows:
        raise SystemExit("No output rows found")
    for i, row in enumerate(output_rows):
...
def main() -> None:
    ...
    if args.mode == "embeddings":
        extra["embedding_dim"] = validate_embeddings(output_rows)
    else:
    ...
    if args.summary_json is not None:
        ...
        if args.require_gpu and int(summary.get("gpu_visible_count", 0)) < 1:
            raise SystemExit("Summary indicates no visible GPU while --require-gpu is set")
