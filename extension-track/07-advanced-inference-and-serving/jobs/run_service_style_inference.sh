...
RUN_NAME="${RUN_NAME:-advanced-inference-service}"
...
set -euo pipefail
cd '$LESSON_DIR'
python scripts/run_service_loop.py --config configs/service.yaml --output-root '$OUT_ROOT' --run-name '$RUN_NAME'
python scripts/collect_metrics.py --config configs/service.yaml --mode service --output-root '$OUT_ROOT' --run-name '$RUN_NAME'
"
