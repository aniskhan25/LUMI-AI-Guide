...
srun singularity exec -B "$SQSH_PATH":/user-software:image-src=/ "$CONTAINER" bash -c '
  set -euo pipefail
  /user-software/bin/python -m torch.distributed.run --nnodes="$SLURM_JOB_NUM_NODES" --nproc_per_node=8 --node_rank="$SLURM_PROCID" --rdzv_id="$SLURM_JOB_ID" --rdzv_backend=c10d --rdzv_endpoint="$MASTER_ADDR:$MASTER_PORT" ds_visiontransformer.py --deepspeed --deepspeed_config ds_config.json
'
