...
#SBATCH --output=/scratch/project_462000131/anisrahm/slurm/comp-large-%j.out
...
FORMAT="${1:-}"
: "${FORMAT:?Usage: sbatch run-scripts/simple-benchmarks/run-comp-large.sh <squashfs|lmdb|hdf5>}"
...
run_benchmark() {
  ...
  srun singularity exec $bind_arg "$CONTAINER" bash -c \
    'if [ -n "${WITH_CONDA:-}" ]; then eval "$WITH_CONDA"; fi; source venv-extension/bin/activate; python run-scripts/simple-benchmarks/compare-dataset-large.py -n 7 -ff "'"$format"'" -N 200000'
}
