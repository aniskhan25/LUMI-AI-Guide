...
Measure whether your allocated GPU resources are used efficiently and identify code-level bottlenecks.

## Assumptions

- You can run the training scripts from earlier chapters.
...
- A profiler trace file `trace.json` is produced in the lesson working directory.
...
- `rocm-smi` shows expected activity for your active GPUs during training.
- A profiler trace file (for example `trace.json`) is generated by the profiled script.
- You can load the trace in [Perfetto](https://ui.perfetto.dev/) and identify dominant operations.

## Troubleshooting

- Interactive attach fails: confirm job state and use the current job id from `squeue --me`.
- Trace file too large for browser memory: profile a shorter interval (single epoch or narrower code section).
- Reported utilization is high but throughput is low: use profiler traces and power draw together, not utilization alone.
